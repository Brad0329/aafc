...
### Phase 8: 관리자 페이지 (ba_office 디자인 복제) (3주)
**목표**: 기존 ASP 관리자 페이지(ba_office)와 동일한 디자인의 관리자 페이지 구축

배경:
- 기존 직원들이 ASP 관리자 화면에 익숙하므로, 재교육 없이 사용할 수 있도록 동일한 UI/UX를 유지한다.
- Django 기본 Admin(/admin/)은 개발용으로 유지하되, 실제 운영 관리자 페이지는 /ba_office/ 경로로 별도 구축한다.

작업:
- [ ] 관리자 공통 레이아웃 (기존 ba_office CSS/JS/이미지 활용)
- [ ] 상단 메뉴: 시스템관리, 회원관리, 상담관리, 수강생관리, 과정관리, 출고관리, REPORT, 포탈관리, 쇼핑몰관리
- [ ] 시스템관리: 공통코드, 설정, 권한 그룹 (9개) 관리
- [ ] 회원관리: 회원 목록/상세, 자녀 목록, 탈퇴회원
- [ ] 상담관리: 상담 목록/상세/답변, 무료체험 관리
- [ ] 수강생관리: 수강생 목록, 입단신청 관리, 변경이력
- [ ] 과정관리: 강좌/구장/코치 관리
- [ ] 출고관리: 교육용품 출고
- [ ] REPORT: 리포트/통계 뷰 (Phase 7 데이터 활용)
- [ ] 포탈관리: 팝업, 공지사항, 이벤트, 게시판 관리
- [ ] 쇼핑몰관리: 상품/주문/배송 관리
- [ ] 관리자 로그인/인증 (권한 체크)

검증: 기존 ASP ba_office 화면과 1:1 비교, 동일 메뉴/기능 동작 확인

---

### Phase 9: NICE 본인인증 + 최종 마무리 (1주)
**목표**: 운영 준비 완료
...
### 검증 원칙
1. **모델 절대 불변**: 검증 시 모델 필드명/타입/관계는 **절대 변경하지 않음** (연쇄 영향 방지)
2. 템플릿/뷰 로직/필터 조건 등 **해당 Phase 내에서 완결되는 수정만** 허용
3. 모델 문제 발견 시 → **수정하지 않고 문서에 기록만** (별도 검토 후 판단)
4. 수정 후 `python manage.py check` + 서버 기동 테스트 필수
5. ASP 원본 파일과 Django 구현을 **1:1 대조**하여 누락/차이점 확인

### 검증 방식
- ASP 원본 파일(`c:\Users\user\Documents\2018_fcjunior\`)과 Django 템플릿/뷰 코드를 비교
- 화면 레이아웃, 데이터 표시, 기능 동작이 ASP와 동일한지 확인
- 브라우저에서 직접 접속하여 수동 테스트

### Phase별 검증 항목

| Phase | 검증 대상 | ASP 원본 파일 | 상태 |
|-------|-----------|---------------|------|
| 1 | 회원가입/로그인/마이페이지/자녀관리/비밀번호변경 | join*.asp, login.asp, mypage*.asp | 미검증 |
| 2 | 구장안내/코칭스태프/강좌목록/운영진인사말 | stadium.asp, coach.asp, lecture*.asp | 미검증 |
| 3 | 수강신청 3단계(자녀→구장→강좌→결제)/결제내역 | fcjoin_step*.asp, pay*.asp | 미검증 |
| 4 | 게시판(목록/상세/글쓰기/댓글)/상담신청/무료체험 | board*.asp, consult*.asp | 미검증 |
| 5 | 쇼핑몰(상품/장바구니/주문/결제/주문조회) | shop*.asp | 미검증 |
| 6 | 포인트조회/알림장/마이페이지메뉴 | point*.asp, alim*.asp | 미검증 |
| 7 | 리포트(전체DATA/코치별/출석/월별)/Excel다운로드 | report*.asp | 미검증 |
...
| 8-3 회원관리 | ✅ 검증 완료 |
| 8-4 상담관리 | ✅ 검증 완료 |
| 8-5 수강생관리 | 개발 완료 (별도 검증 커밋 없음, 8-5 보완 커밋으로 대체) |
| 8-5-2 수강생등록/대기자 | 개발 완료 |
| 8-6 과정관리 | 개발 완료 |
...
| Phase | 내용 | 기간 | 상태 |
|-------|------|------|------|
| 0 | 프로젝트 세팅 + DB 마이그레이션 | 1주 | ✅ 완료 |
| 1 | 회원 + 인증 | 2주 | ✅ 완료 (미검증) |
| 2 | 강좌 + 구장 + 코치 | 1주 | ✅ 완료 (미검증) |
| 3 | 수강신청 + 결제 ★ | 3주 | ✅ 완료 (미검증) |
| 4 | 게시판 + 상담 | 1주 | ✅ 완료 (미검증) |
| 5 | 쇼핑몰 | 2주 | ✅ 완료 (미검증) |
| 6 | 포인트 + 알림 + SMS | 1주 | ✅ 완료 (미검증) |
| 7 | 리포트 + 통계 | 2주 | ✅ 완료 (미검증) |
| 8 | 관리자 페이지 (ba_office 디자인 복제) | 3주 | ✅ 완료 (일부 보류) |
| 9 | NICE 인증 + 최종 마무리 | 1주 | ✅ 항목 1~4 완료 (5~7 보류) |
| **합계** | | **약 17주** |
